...
for((input=0; input < 200; input++))
        do
		for((input1=0; input1 < 200; input1++))
        	do
                	echo "/tmp/nt284/mountdir/file$input1" >> "/tmp/nt284/mountdir/file$input1"
...
for((input2=0; input2 < 200; input2++))
        do
...
read MYVAR
for((input3=0; input3 < 200; input3++))
        do
                #echo "/tmp/nt284/mountdir/file$number" >> "/tmp/nt284/mountdir/file$number"
                #wc "/tmp/nt284/mountdir/file$number"
